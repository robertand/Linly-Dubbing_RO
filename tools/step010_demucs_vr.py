...
current_model_config = {}  # Variable to store current model configuration


def init_demucs():
    """
    Initialize Demucs model.
    If model is already initialized, return immediately without reloading.
    """
...
def load_model(model_name: str = "htdemucs_ft", device: str = 'auto', progress: bool = True,
    ...
    """
    Load Demucs model.
    If model with same configuration is already loaded, reuse existing model.
    """
    global separator, model_loaded, current_model_config

    if separator is not None:
        # Check if reload is needed (different configuration)
        requested_config = {
        ...
        if current_model_config == requested_config:
            logger.info(f'Demucs model already loaded with same configuration, reusing existing model')
            return separator
        else:
            logger.info(f'Demucs model configuration changed, reloading...')
...
def separate_audio(folder: str, model_name: str = "htdemucs_ft", device: str = 'auto', progress: bool = True,
    ...
    try:
        # Ensure model is loaded and configuration is correct
        ...
        try:
            ...
            # Attempt to reload model once on error
            release_model()
            load_model(model_name, device, progress, shifts)
        ...
        t_end = time.time()
        logger.info(f'Audio separation complete in {t_end - t_start:.2f} seconds')
        ...
        # Release model resources on error and re-raise
        release_model()
        raise
...
def separate_all_audio_under_folder(root_folder: str, model_name: str = "htdemucs_ft", device: str = 'auto',
    ...
    """
    Separate all audio files under a folder.
    """
    global separator
    ...
    try:
        ...
        logger.info(f'Completed separation for all audio under: {root_folder}')
        return f'Completed all audio separation: {root_folder}', vocal_output_path, instruments_output_path

    except Exception as e:
        logger.error(f'Error during audio separation: {str(e)}')
        # Release model resources on error
        release_model()
        raise
